...
function Navbar({ aba, setAba }) {
  return (
    <header className={`${colors.bgSecondary} border-b ${colors.borderNav} px-6 py-4`}>
      <div className="max-w-7xl mx-auto flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className={`flex gap-1 p-1 ${colors.bgNavInput} ${rounded.lg} self-start sm:self-auto`}>
          {["Dashboard", "Agendamentos"].map((item) => (
            <button
              key={item}
              onClick={() => setAba(item)}
              className={`px-4 py-2 ${rounded.md} ${typography.body} transition-all duration-200 ${
                aba === item
